  ...
  describe(scenarioName, () => {
    ...
    if (changesets) {
      for (const [index, changeset] of changesets.entries()) {
        const name = changeset.changes.length === 1
          ? changeset.changes[0]!.path
          : index;

        test(
          `should match normal build after file changes - ${name}`,
          async () => {
            // Prime cache
...
function withoutDomainExpansion(config: AstroInlineConfig = {}): AstroInlineConfig {
  ...
  return {
    ...
    integrations: config.integrations
      .flat()
      .filter(int => int && int.name !== INTEGRATION_NAME),
  }
}
