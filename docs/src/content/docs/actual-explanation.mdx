...
After readint the Tale of the Three Mages, you might be a little confused, so here's an actual explanation of how 
Domain Expansion works under the hood.

## How Astro builds your site

Whenever you run `astro build`, Astro will essentially "request" your pages internally and save the resulting HTML.
This happens with two functions, `$$render` and `$$renderComponent`. The first one is used to render the whole page,
while the latter is used to render components. You can see the output of these functions on 
[the live Astro compiler](https://live-astro-compiler.vercel.app/). The output of a page from the compiler will look
somewhat like this:
...
while the `$$render` template tag is used to render the whole page or smaller bits which don't require components.
...
The cache is saved in `node_modules/.domain-expansion`. Additionally, whenever the Astro compiler receives an update, the entire
